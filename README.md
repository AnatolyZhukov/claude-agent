...
- When `SKILL_ID` is set, the model can consult the `metric-aggregation-rules` Skill via Anthropic's `code_execution` tool — but that sandbox has no access to the real database, so `ask()` rejects any answer that uses `code_execution` for anything beyond reading the Skill file itself (the vetting logic lives in `code_execution_guard.py`), instead of risking a fabricated answer. Since the detour is usually an attempt to *draw* something after the real tools already returned the data, the turn is rescued rather than discarded: the model is told the tool isn't available and asked to write the answer from what it already has. That replaces an earlier shortcut which handed the raw tool output to the user as the answer — genuine data, but reaching the chat as a wall of unlabelled numbers with the tools' own internal instructions embedded in it. If the retry detours again, the user gets a plain explanation; the charts that were already produced render either way. A request timeout (60s) also keeps a question that goes down that path from hanging the app.
...
`get_matrix_data` crosses two of those dimensions for one metric and returns a heatmap — `revenue_per_customer` by month and category, say. Without it, "show me a heatmap by month and category" went through raw SQL, came back as a flat table, and got pasted into the answer as an ASCII grid. It keeps the largest 24 row values and 12 column values (a heatmap stops being readable well before it stops being renderable) and says so when it drops any; an empty cell means that combination never occurred, which is why the matrix carries `None` there rather than `0.0`. Rendering reuses the cohort heatmap's shading and markup so the two read as one thing.
